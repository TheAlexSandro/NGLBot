...
function tagTambah(kata, isi) {
  ...
  // jika tag belum ada, kita tambahkan
    ...
    return '✅ Akun '+kata+' sudah ditambahkan';
  }
}
...
function hapusTag(kata) {
  ...
  if (barisTag) {
    ...
    return '✅ Akun <code>'+kata+'</code> telah dihapus.';
  } else {
    // kalau gak ketemu, tampilkan saja pesan error
    return 'ℹ️ Akun <code>'+kata+'</code> tidak ditemukan untuk dihapus.';
  }
}
...
function tagList() {
  ...
  let isiKolom = sheet.getRange(baris, ht, sheet.getLastRow()).getValues();
